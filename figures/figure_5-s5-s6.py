...
# =============================================================================
# Genotype-specific dataset definitions
# =============================================================================
# Each dictionary contains:
#  - label_show: readable label used in plots
#  - path: base folder for model fit files and errors
#  - path_fish: aggregated HDF5 file with empirical fish data for that mutation
#  - path_simulation: aggregated HDF5 file with synthetic/model data for that mutation
models_in_mutation_list_figure_5 = {
    "group_scn1": [
        {"label_show": r"$scn1lab^{+/+}_{allele 1}$",
         "path": f"{path_data}/scn1lab_NIBR_20200708/wt",
         "path_fish": f"{path_data}/scn1lab_NIBR_20200708/wt/data_fish_all-wt.hdf5",
         "path_simulation": f"{path_data}/scn1lab_NIBR_20200708/wt/data_synthetic_fish_all-wt.hdf5"},
        {"label_show": r"$scn1lab^{+/—}_{allele 1}$",
         "path": f"{path_data}/scn1lab_NIBR_20200708/het",
         "path_fish": f"{path_data}/scn1lab_NIBR_20200708/het/data_fish_all-het.hdf5",
         "path_simulation": f"{path_data}/scn1lab_NIBR_20200708/het/data_synthetic_fish_all-het.hdf5"},
    ],
    "group_scn2":[
# models_in_mutation_list_figure_s5 = [
        {"label_show": r"$scn1lab^{+/+}_{allele 2}$",
...
# models_in_mutation_list_figure_s6 = [
        {"label_show": r"$disc^{+/+}$",
         ...
         "path_simulation": f"{path_data}/disc1_hetnix/wt/data_synthetic_fish_all-wt.hdf5"},
        {"label_show": r"$disc^{+/—}$",
         "path": f"{path_data}/disc1_hetnix/het",
         "path_fish": f"{path_data}/disc1_hetnix/het/data_fish_all-het.hdf5",
         "path_simulation": f"{path_data}/disc1_hetnix/het/data_synthetic_fish_all-het.hdf5"},
        {"label_show": r"$disc^{—/—}$",
...
models_in_mutation_list_to_show = models_in_mutation_list_figure_5
figure_label = "5"  # number of the figure (alternatives are s5 or s6)

# =============================================================================
# Load data files for each mutation group (empirical + synthetic)
# =============================================================================
...
for models_in_mutation_list in models_in_mutation_list_figure_5.values():
    for m in models_in_mutation_list:
        try:
...
# =============================================================================
# Purpose:
#  - For each mutation group, read per-model 'error_fish_*.hdf5' files and plot the
...
if show_loss_reduction:
    ...
    for models_in_mutation_list in models_in_mutation_list_figure_5.values():
        ...
        for i_mutation, models_in_mutation in enumerate(models_in_mutation_list):
            ...
            for path_error in Path(models_in_mutation["path"]).glob("error_fish_*.hdf5"):
            ...
            plot_title = f"{models_in_mutation['label_show']}\n" fr"n={len(loss_list)}"
            plot_loss = fig.create_plot(plot_label=style.get_plot_label() if (i_group == 0 and i_mutation == 0) else None,
                                        plot_title=plot_title,
                                        xpos=xpos, ypos=ypos, plot_height=plot_height, plot_width=plot_width,
                                        ymin=-1, ymax=20, yticks=[0, 10, 20] if (i_group == 0 and i_mutation == 0) else None,
                                        yl="Loss" if (i_group == 0 and i_mutation == 0) else None,
                                        xmin=0.5, xmax=2.5,
...
# =============================================================================
# Purpose:
#  - For each mutation group plot percentage correct vs coherence for both empirical
...
if show_psychometric_curve:
    ...
    for models_in_mutation_list in models_in_mutation_list_figure_5.values():
        i_group += 1
        for i_m, m in enumerate(models_in_mutation_list):
            df_data = m["df_data"]
            ...
            # Create a small panel per mutation group, optionally showing y-axis for the first panel
            plot_0 = fig.create_plot(plot_label=style.get_plot_label() if (i_group == 0 and i_m == 0) else None, xpos=xpos, ypos=ypos, plot_height=plot_height,
                                     ...
                                     yl="Percentage\ncorrect swims (%)" if (i_group == 0 and i_m == 0) else None,
                                     ymin=45, ymax=100, yticks=[50, 100] if (i_group == 0 and i_m == 0) else None, hlines=[50])
...
if show_coherence_vs_interbout_interval:
    ...
    for models_in_mutation_list in models_in_mutation_list_figure_5.values():
        ...
        for i_m, m in enumerate(models_in_mutation_list):
            ...
            # Create plot panel(s) across mutation groups; show y-axis label only for first panel
            plot_0 = fig.create_plot(plot_label=style.get_plot_label() if (i_group == 0 and i_m == 0) else None, xpos=xpos, ypos=ypos,
                                     ...
                                     xticks=[int(p) for p in ConfigurationExperiment.coherence_list],  # if i_m == len(models_in_mutation_list)-1 else None,
                                     yl="Interbout\ninterval (s)" if (i_group == 0 and i_m == 0) else None,
                                     ymin=0, ymax=2, yticks=[0, 1, 2] if (i_group == 0 and i_m == 0) else None)
...
# =============================================================================
# Purpose:
#  - For each mutation group, gather parameter medians across fitted models (model_*_fit.hdf5)
#  - Build histograms of medians (to visualize cross-individual variability)
#  - Perform bootstrap/resampling to estimate median quantiles per mutation group
...
if show_distribution_parameters:
    ...
    plot_width_here = style.plot_width * 1.2
    ...
    for models_in_mutation_list in models_in_mutation_list_figure_5.values():
        ...
        distribution_trajectory_dict = {p["label"]: np.zeros((number_bins_hist, len(models_in_mutation_list))) for p in ConfigurationDDM.parameter_list}
        raw_data_dict_per_fish = {p["label"]: {i_mutation: {} for i_mutation in range(len(models_in_mutation_list))} for p in ConfigurationDDM.parameter_list}
        raw_data_dict = {p["label"]: {i_mutation: [] for i_mutation in range(len(models_in_mutation_list))} for p in ConfigurationDDM.parameter_list}
        median_groups = {p["label"]: np.zeros(len(models_in_mutation_list)) for p in ConfigurationDDM.parameter_list}
        quantiles_groups = {p["label"]: np.zeros((len(models_in_mutation_list), 3)) for p in ConfigurationDDM.parameter_list}

        # Loop through mutation groups, read models, and compute per-model medians
        for i_mutation, models_in_mutation in enumerate(models_in_mutation_list):
            model_dict = {}
            path_dir = Path(models_in_mutation["path"])
            ...
            for i_model, id_model in enumerate(model_dict.keys()):
                ...
                for i_p, p in enumerate(ConfigurationDDM.parameter_list):
                    ...
                    # Save per-model medians to raw_data_dict_per_fish and per-mutation raw_data_dict
                    if id_model not in raw_data_dict_per_fish[p["label"]][i_mutation].keys():
                        raw_data_dict_per_fish[p["label"]][i_mutation][id_model] = [p_median]
                    else:
                        raw_data_dict_per_fish[p["label"]][i_mutation][id_model].append(p_median)

                    raw_data_dict[p["label"]][i_mutation].append(p_median)

            # For each parameter: compute median across models in this mutation group and bootstrap quantiles
            for i_p, p in enumerate(ConfigurationDDM.parameter_list):
                median_groups[p["label"]][i_mutation] = np.median(raw_data_dict[p["label"]][i_mutation])
                ...
                quantiles_groups[p["label"]][i_mutation, :] = np.quantile(median_list, [0.05, 0.5, 0.95])
                print(f"group {models_in_mutation['label_show']} | {p['label_show']}: {quantiles_groups[p['label']][i_mutation, 1]:.05f} [{quantiles_groups[p['label']][i_mutation, 0]:.05f}, {quantiles_groups[p['label']][i_mutation, 2]:.05f}]")
            ...
            for i_p, p in enumerate(ConfigurationDDM.parameter_list):
                ...
                # Collect histogram values across mutations (columns = mutant groups)
                distribution_trajectory_dict[p["label"]][:, i_mutation] = hist_model_parameter_median_dict[p["label"]]

        # =============================================================================
        # Plot D1 — Histogram panels for each parameter x mutation group (small panels grid)
        # =============================================================================
        # Draw a grid: for each mutation (columns) and parameter (rows), show the distribution of fitted medians.
        for i_mutation in range(len(models_in_mutation_list)):
            for i_p, p in enumerate(ConfigurationDDM.parameter_list):
                plot_n = fig.create_plot(plot_label=style.get_plot_label() if i_p == 0 and i_mutation == 0 else None, xpos=xpos, ypos=ypos,
                                         plot_height=plot_height_here, plot_width=plot_width_here,
                                         yl="Percentage fish (%)" if i_p == 0 and i_mutation == 0 else None, ymin=0, ymax=50, yticks=[0, 50] if i_p == 0 and i_mutation == 0 else None,
                ...
                xpos = xpos + padding_small + plot_width_here
        ...
        # subplot showing quantile lines per mutation group (used for comparison / annotation).
        plot_list = []
        for i_p, p in enumerate(ConfigurationDDM.parameter_list):
            plot_n = fig.create_plot(plot_label=style.get_plot_label() if i_p==0 else None, xpos=xpos, ypos=ypos,
                                     ...
                                     ymin=0, ymax=len(models_in_mutation_list) + 1,
                                     yticks=np.arange(len(models_in_mutation_list), 0, -1) if i_p == 0 else None,
                                     yticklabels=[g["label_show"] for g in models_in_mutation_list] if i_p == 0 else None,
            ...
            for i_group, models_in_group in enumerate(models_in_mutation_list):
                plot_n.draw_line(quantiles_groups[p["label"]][i_group, :],
                                 np.ones(quantiles_groups[p["label"]].shape[1]) * (len(models_in_mutation_list) - i_group),
                                 lc=palette[i_p])
        ...
        # =============================================================================
        # For each parameter, compute pairwise comparisons between mutation groups using resampling:
        ...
        pairs_to_compare_list = list(itertools.combinations(range(len(models_in_mutation_list)), 2))
        for i_p, p in enumerate(ConfigurationDDM.parameter_list):
            ...
            for pairs_to_compare in pairs_to_compare_list:
                ...
                print(f"{models_in_mutation_list[pairs_to_compare[0]]['label_show']} vs {models_in_mutation_list[pairs_to_compare[1]]['label_show']} | p = {p_value}")
                ...
                if p_value < threshold_p_value_significant:
                    ...
                    y_sig_array = np.array(
                        (len(models_in_mutation_list) - pairs_to_compare[0], len(models_in_mutation_list) - pairs_to_compare[1]))
            ...
            if is_sig:
                plot_n.draw_text(x=x_sig + parameter_range / 6, y=np.ceil(len(models_in_mutation_list) / 2), text="*",
                                 textlabel_rotation=-90)
        xpos = xpos_start
        ypos = ypos - (plot_height + padding*1.5)
