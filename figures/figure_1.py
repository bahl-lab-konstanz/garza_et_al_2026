...
path_data = path_dir / "base_dataset_5dpfWT"
...
# =============================================================================
# Flags to toggle specific figures on/off
# =============================================================================
...
# Load datasets for individual fish and combined dataset ("all")
df_dict = {}
all_label = "all"
...
for fish in fish_to_include_list:
    ...
    df_dict[fish] = df  # (optionally could preprocess here)
...
# =============================================================================
# Plot 1 — Trial structure visualization
# =============================================================================
if show_trial_structure:
    ...
    # Get sample data for the first example fish
    fish_0_label = ConfigurationExperiment.example_fish_list[0]
    df_sample = df_dict[fish_0_label]

    # Define time windows (stimulus vs rest)
    time_stimulus = np.arange(ConfigurationExperiment.time_start_stimulus,
                              ConfigurationExperiment.time_end_stimulus+2*ConfigurationDDM.dt,
                              ConfigurationDDM.dt)
    ...
    # Loop over experiments and stimulus parameters
    for i_ex, ex in enumerate(experiment_list):
        for i_param, parameter in enumerate(ex["analysed_parameter_list"]):
            ...
            plot_section = fig.create_plot(
                plot_label=style.get_plot_label() if i_param == 0 else None,
                xpos=xpos,
                ypos=ypos_here - (padding_vert_here + plot_height_here) * number_plots - height_drift,
            ...
            # Filter dataset for this parameter + trial
            ...
            # Construct input stimulus signal
            ...
            # Draw baseline and stimulus trace
            ...
            # Color decision events depending on whether they occur during stimulus
            ...
            plot_section.draw_scatter(decision_time_list,
                                      0.5+np.zeros(len(decision_time_list)),
                                      pt="|", ec=color_scatter, pc=color_scatter)
...
# =============================================================================
# Plot 2 — Distribution of orientation changes
# =============================================================================
if show_distribution_change_angles:
    ...
    plot_0 = fig.create_plot(
        ...
        errorbar_area=False,
        xmin=min_angle, xmax=max_angle, ymin=0, ymax=ymax,
        xticks=[min_angle, 0, max_angle], xl='Orientation change (°)',
        yticks=[0, ymax], yl='Percentage swims (%)',
        vlines=[0],
        vspans=[
            [min_angle, -threshold_side, style.palette["correct_incorrect"][1], 1],
            [threshold_side, max_angle, style.palette["correct_incorrect"][0], 1]
        ]
    )

    # Filter bouts by time and angle
    query_time_angle = (
        f"start_time > {ConfigurationExperiment.time_start_stimulus} "
        f"and end_time < {ConfigurationExperiment.time_end_stimulus} "
        f"and estimated_orientation_change < {max_angle} "
        f"and estimated_orientation_change > {min_angle}"
    )
    df_filtered = df.query(query_time_angle)

    # Plot distributions per coherence level
    for i_coherence, coherence in enumerate(ConfigurationExperiment.coherence_list):
        ...
        # Flip orientation values if required
        df_coh['flipped_estimated_orientation_change'] = BehavioralProcessing.flip_column(
            df_coh, 'estimated_orientation_change', inverted_direction=inverted_direction
        )

        if len(df_coh['flipped_estimated_orientation_change'].dropna()) == 0:
            print(f"WARNING | distribution of angle changes | no values for coherence {coherence}")
        else:
            ...
            angle_start_list, angle_center_list, angle_end_list = StatisticsService.get_window_list_from_data(
                angle_list, window_number=angle_list_length, center=0
            )
            ...
            for angle_index in range(len(angle_center_list)):
                angle_value = len([angle for angle in df_coh['flipped_estimated_orientation_change']
                                   if angle_start_list[angle_index] <= angle <= angle_end_list[angle_index]]) \
                              / len(df_coh['flipped_estimated_orientation_change'])
                angle_distribution[angle_index] = angle_value

            # Plot distribution
            plot_0.draw_line(angle_center_list, angle_distribution * 100,
                             lc=style.palette["stimulus"][i_coherence],
...
# =============================================================================
# Plot 3 — Example fish trajectory visualization
# =============================================================================
if show_fish_trajectory:
    plot_height_here = 2
    plot_width_here = 2

    # Draw a reference circle (arena boundary)
    x = np.linspace(-1.0, 1.0, 100)
    y = np.linspace(-1.0, 1.0, 100)
    X, Y = np.meshgrid(x, y)
    F = X ** 2 + Y ** 2 - 0.6

    # Select trial/fish to visualize
    ...
    plot_0.ax.contour(X, Y, F, [0], colors="k")  # arena outline

    # Loop through selected trials and plot trajectory
    for trial_id in trial_id_list:
        ...
        # Draw path (line) and endpoints (scatter, colored by correctness)
        ...
        plot_0.draw_scatter(df_filtered[x_position_column], df_filtered[y_position_column],
                            pc=color_list, ec=color_list, alpha=0.5)

    # Update position for next plots
    ypos = ypos - plot_height_here - padding
    xpos = xpos_start

# =============================================================================
# Common layout parameters for multi-row plots
# =============================================================================
...
# =============================================================================
# Plot 4 — Psychometric curve (accuracy vs coherence)
# =============================================================================
...
# =============================================================================
# Plot 5 — Coherence vs Interbout Interval (IBI)
# =============================================================================
xpos = xpos_start
ypos = ypos - padding - plot_height
...
if show_coherence_vs_interbout_interval:
    ...
    # Compute interbout intervals across fish
    parameter_list, interbout_interval_list, std_correct_bout_list = BehavioralProcessing.compute_quantities_per_parameters_multiple_fish(
        df_filtered, analysed_parameter=coherence_label, column_name=ConfigurationExperiment.ResponseTimeColumn)
    ...
    plot_0 = fig.create_plot(xpos=xpos, ypos=ypos,
                             ...
                             yl="Interbout interval (s)",
                             ymin=0, ymax=2,
                             yticks=[0, 1, 2])

    # Individual fish curves
    for i_id, id in enumerate(id_fish_list):
        df_fish = df_filtered_all.xs(id, level="experiment_ID")
        df_fish_filtered = df_fish[df_fish[coherence_label].isin(ConfigurationExperiment.coherence_list)]
        parameter_list, correct_bout_list, std_correct_bout_list = BehavioralProcessing.compute_quantities_per_parameters(
    ...
    plot_0.draw_line(x=parameter_list, y=interbout_interval_list, lc="k", lw=1, line_dashes=line_dashes)
...
# =============================================================================
# Plot 6 — Interbout interval distributions
# =============================================================================
xpos = xpos_start
...
if show_rt_distributions:
    ...
    palette = style.palette["stimulus"]
    x_limits = [0, 2]   # IBI axis (seconds)
    y_limits = [0, 0.2] # Density axis

    plot_height_here = plot_height_small * 3
    plot_width_here = plot_width * 0.7
    plot_section = {}

    # Remove merged dataset (use only per-fish data)
    df_dict.pop(all_label, None)

    # Loop through fish and coherence values
    for i_k, k in enumerate(df_dict.keys()):
        df = df_dict[k]
        for i_param, parameter in enumerate(ConfigurationExperiment.coherence_list):
            # Create subplot for each coherence level
            line_dashes = None
            plot_title = None
            if i_k == 0:
                if i_param == 0:
            ...
            plot_section[i_param] = fig.create_plot(
                plot_label=style.get_plot_label() if i_k == 0 and i_param == 0 else None,
            ...
            if i_param == len(ConfigurationExperiment.coherence_list) - 1 and i_k == len(df_dict)-1:
                ...
                plot_section[i_param].draw_line((x_location_scalebar, x_location_scalebar + 0.5),
            ...
            # Select correct/incorrect responses
            df_filtered = df[df[coherence_label] == parameter].query(query_time)
            duration = np.sum(BehavioralProcessing.get_duration_trials_in_df(
                df_filtered,
            ...
            # Compute histograms for correct responses
            ...
            # Compute histograms for incorrect responses
            ...
            # Plot distributions (correct above 0, incorrect mirrored below 0)
            plot_section[i_param].draw_line(data_hist_time_corr, data_hist_value_corr,
                                            lc=style.palette["correct_incorrect"][0],
                                            lw=0.75, line_dashes=line_dashes)
            plot_section[i_param].draw_line(data_hist_time_err, -1 * data_hist_value_err,
                                            lc=style.palette["correct_incorrect"][1],
                                            lw=0.75, line_dashes=line_dashes)
